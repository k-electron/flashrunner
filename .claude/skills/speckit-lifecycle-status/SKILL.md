...
metadata:
  ...
  source: lifecycle:commands/speckit.lifecycle.status.md
---
...
```bash
.specify/extensions/lifecycle/scripts/lifecycle-engine.py status "$@"
```
...
1. Run `.specify/extensions/lifecycle/scripts/lifecycle-engine.py status` to query the state machine.
