...
- [X] T049 [P] **Verify the Cloudflare Pages project's build command and `NODE_VERSION`, and record the result in the repo**, per **Constitution III** ("Runtime version pinned identically in `.nvmrc`, `engines`, CI workflow, and Pages env" and "CI build uses the same command Cloudflare Pages runs") (`partial`). Three of the four pins are verified in-repo and agree: `.nvmrc` = `26.7.0`, `package.json` `engines.node` = `26.7.0`, and `.github/workflows/ci.yml` reads `node-version-file: .nvmrc`. The fourth lives in the Cloudflare dashboard and is recorded **nowhere in this repository** — `specs/000-scaffold/tasks.md` has no Pages task at all, and [research.md](./research.md) stating the intended `NODE_VERSION = 26.7.0` is a plan, not evidence. [plan.md](./plan.md) § Constitution Check asserts the propagation as fact. Check the dashboard, confirm the build command is `npm run build` and `NODE_VERSION` is `26.7.0`, correct either if it has drifted, and write what you found into the repo so the next audit does not have to leave this open. Read-only repo access structurally cannot close this one.

  **Resolved 2026-08-23 by the maintainer reading the dashboard.** Build system **version 3**, and **no `NODE_VERSION` variable exists** in either environment. A v3 build image reads `.nvmrc`, so with nothing overriding it the pin propagates by construction — the build log confirms `Installing nodejs 26.7.0`, matching `.nvmrc`, `engines.node`, and the workflow's `node-version-file`.

  **This corrects the premise of the task.** There is no *fourth place* holding a second copy of the version that could drift; there are three in-repo pins and a host that derives from the first. The absence of the variable is the guarantee, which is stronger than a manual four-way sync. `plan.md`'s claim that the pin "propagates to … Pages `NODE_VERSION`" was therefore false as written, and is corrected in this commit.

  The build-command half rests on weaker evidence and is recorded as such: the deployed assets are byte-identical to a local `npm run build`, and the output directory resolves to `dist`. That does not recover the configured command *string* — `tsc -b` emits nothing under `noEmit`, so `npm run build`, `vite build`, and `npx vite build` all produce identical bytes. The toolchain-mismatch risk Principle III guards against is closed; the literal string is not independently confirmed.

  Per the maintainer's standing constraint, **no host-specific file was added to the repo** — the finding lives here and in the corrected `plan.md` line.
...
- [X] T051 **Confirm which error a full `localStorage` actually throws in every target browser, and widen the quota check if it differs**, in `src/storage/safeStorage.ts:64-71`, per **Constitution II** ("Handle `QuotaExceededError` explicitly and surface it") (`partial`). Classification matches on `error.name === 'QuotaExceededError'` alone; anything else falls through to `reason: 'unavailable'`, and `src/routes/Run.tsx:81` gates the "progress is not being saved" banner on `'quota-exceeded'` only — so a mismatch means a learner with a full profile is silently lied to, which is the exact outcome FR-028's handling exists to prevent. **This is a lead, not a confirmed defect**: Firefox has historically thrown a `DOMException` named `NS_ERROR_DOM_QUOTA_REACHED` (code 1014) rather than `QuotaExceededError` (code 22), and whether that is still true of current Firefox was **not verified** — it cannot be tested from this environment, since the suite never reaches a real store at all (see T047). Check it against the evergreen browsers plan.md § Target Platform names before changing anything. If the name does differ, widen to cover the code as well as the name; if it does not, record that and close this task without a code change.

  **Resolved 2026-08-23 with no code change.** The check is correct as written. Verified at engine source: Gecko (`LSObject.cpp` → `NS_ERROR_DOM_QUOTA_EXCEEDED_ERR`), WebKit (`Storage.cpp`), and Blink (`storage_area.cc`) all produce `name === 'QuotaExceededError'` with `code === 22`, matching HTML `setItem` step 4. The `NS_ERROR_DOM_QUOTA_REACHED` / 1014 behaviour was real but is Firefox ≤ 63 — Gecko bug 1482194 fixed it in Firefox 64, released 2018-12-11, which predates this project's evergreen baseline; confirmed by diffing the `FIREFOX_63_0_RELEASE` and `FIREFOX_64_0_RELEASE` tags. WebIDL's 2025 upgrade of `QuotaExceededError` to a derived interface preserves `name` and `code` and explicitly recommends `name`. Widening would be speculative hardening (Principle VI); `instanceof QuotaExceededError` would be worse, being Blink 138+ only. The finding is recorded on `isQuotaExceeded` in `src/storage/safeStorage.ts`, where anyone tempted to widen it will read it — not in `contracts/storage.md`, whose Hostile-storage row states behaviour and is unchanged by this.

  Noted while researching, and **not** acted on: `jsdom@30.0.1` accepts a `storageQuota` option and throws a real `QuotaExceededError`, so this task's aside that quota is untestable in this environment is wrong. That is T047's territory, already closed.
