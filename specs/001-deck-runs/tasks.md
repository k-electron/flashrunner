...
- [X] T011 [P] Create `src/storage/migrations.ts`: `CURRENT_SCHEMA_VERSION = 1`, an ordered migration registry keyed by version (ships empty — version 1 is the baseline), and `runMigrations(parsed: unknown): unknown` applying them in order. Its purpose today is that the first real bump has an obvious home and an established test pattern, per [contracts/storage.md § Migrations](./contracts/storage.md#migrations).
