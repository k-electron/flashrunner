...
deck. Rungs above the highest completed one are visible but not startable.
...
- [ ] T029 [US2] Build `src/routes/DeckLadder.tsx` for `/deck/:deckId`: list every rung in order with its label, mark completed ones, render rungs above the highest completed one as visible but not startable (FR-015, US2 scenario 4), show mastery when derived, and link back to the deck list (FR-034). Progress comes from `readDeckRecord` and `src/decks/ladder.ts`; this file derives nothing itself.
