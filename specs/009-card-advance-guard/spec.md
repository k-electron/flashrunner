...
1. **Given** a run showing card A, **When** an outcome is pressed, **Then** the card
   and the outcome buttons move as a single group while card B is presented — they
   do not animate separately or on different timings.
2. **Given** an outcome is pressed, **When** the buttons dim on that press,
   **Then** they recover to full strength over the course of the transition and
   reach it on the frame they become pressable again.
...
5. **Given** the last card of a cycle is marked, **When** the run-complete screen
   replaces the card, **Then** no transition is left running and the completion
   controls are pressable straight away.
...
- Marking the final card of a run ends the guard as the run-complete screen
  appears — the completion controls are never briefly unpressable.
- "Start over" and "Leave this run" sit below the card and are unaffected by the
  guard, in either direction: pressing an outcome does not lock them, and they do
  not start a guard window of their own.
...
- A run resumed from storage begins with no guard active — the first card is
  pressable immediately on arrival.
...
- **FR-005**: The system MUST play a visible transition while the guard window is
  open, in which the card and the outcome buttons move as one group on a single
  shared timing. They MUST NOT animate independently or on different curves.
...
- **FR-005b**: On press, the outcome buttons MUST dim and then recover to full
  strength across the window, reaching it as the transition ends. This recovery is
  the only progress cue: the system MUST NOT show a countdown, numeric timer, or
  spinner, and MUST NOT give the buttons a greyed-out or switched-off appearance.
- **FR-005c**: The progress bars MUST animate their fill from the old value to the
  new one over the same window, concurrently with the card transition, rather than
  jumping to the new value.
- **FR-006**: The guard window and the transition MUST begin and end together. The
  controls become pressable again on the transition's final frame — not earlier
  (which would leave motion still playing over a live control) and not later
  (which would leave a finished screen briefly dead).
- **FR-007**: One named duration MUST govern the guard window, the card-block
  transition, the button colour recovery, and the progress-bar fill together, so a
  single edit retimes all of them and they cannot drift apart.
...
- **FR-008**: The system MUST remain correct at a duration of zero — one press
  marks exactly one card.
- **FR-009**: Reaching the end of a run MUST leave no guard in force on the
  run-complete screen.
- **FR-010**: The guard MUST NOT be persisted, and MUST NOT be active when a run
  is entered or resumed.
...
- **FR-012**: The guard MUST NOT block "Start over" or "Leave this run". These sit
  below the card block, do not change between cards, and are not part of the moving
  group.
...
- **SC-003**: On marking a card, every element that changes — card, buttons, both
  progress bars — is visibly in motion for the same interval, and all of them come
  to rest at the same moment. Nothing on the screen snaps while something else eases.
- **SC-004**: Retiming the whole transition is a one-value edit; adjusting its
  travel or easing is a one-value edit in the same place. Neither requires a change
  to any test that describes behaviour rather than timing.
...
- **Nothing on the run screen changes on its own timing.** The card block's
  motion, the button recovery, and the bar fill all run over the one window. A
  press landing inside that window is discarded outright, so motion in the block
  can never cause a *wrong* mark — only a press that does nothing.
