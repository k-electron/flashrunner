...
- [X] T013 [US2] Add the resume tests to `src/routes/Run.test.tsx` — **SC-005**: a run interrupted and resumed at several different points presents a card sequence identical to one uninterrupted playthrough under the same stub. **SC-006**: across a resumed run, no card marked "Got it" is ever presented again
